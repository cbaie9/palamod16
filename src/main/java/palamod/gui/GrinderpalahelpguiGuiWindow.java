...
@OnlyIn(Dist.CLIENT)
public class GrinderpalahelpguiGuiWindow extends ContainerScreen<GrinderpalahelpguiGui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = GrinderpalahelpguiGui.guistate;

	public GrinderpalahelpguiGuiWindow(GrinderpalahelpguiGui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("palamod:textures/grinderpalahelpgui.png");
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 5, this.guiTop + 159, 50, 20, new StringTextComponent("next"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new GrinderpalahelpguiGui.ButtonPressedMessage(0, x, y, z));
				GrinderpalahelpguiGui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 63, this.guiTop + 159, 45, 20, new StringTextComponent("menu"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new GrinderpalahelpguiGui.ButtonPressedMessage(1, x, y, z));
				GrinderpalahelpguiGui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 124, this.guiTop + 159, 45, 20, new StringTextComponent("back"), e -> {
			if (true) {
				PalamodMod.PACKET_HANDLER.sendToServer(new GrinderpalahelpguiGui.ButtonPressedMessage(2, x, y, z));
				GrinderpalahelpguiGui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
	}
}
